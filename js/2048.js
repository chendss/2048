...
var cleanTarget = function () {
    targetDict.exclude = []
    targetDict.loading = false
    targetDict.eventList = []
}
...
var randomCallBack = function (n = '2') {
    ...
    let emptyNumbers = list.filter(item => item.textContent === '')
 ...
 * @param {*} dictList
 */
var addEventQueue = function (startItem, endItem, cells, dictList) {
    //  给目标元素附上类和值
    //  当所有动画执行结束调用清空函数
    let flag = true
    const transitionend = function (event, that, resolve, terval) {
        if (event.target === that && flag) {
            flag = false
            log('动画完结')
...
var unBindEvent = function () {
    targetDict.eventList.forEach(item => {
        item.ele.removeEventListener('transitionend', item.fun)
    })
}
...
var transform = function (dict, cells, dictList) {
    ...
    addEventQueue(startItem, endItem, cells, dictList)
}

var transformed = async function (dictList, cells) {
    for (let dict of dictList) {
        transform(dict, cells, dictList)
    }
    let promiseArray = targetDict.eventList.map(evn => {
        let { ele, fun } = evn
...
var move = async function (numbers, cells) {
    targetDict.loading = true
    let dictList = moveDictList(numbers)
    dictList = dictList.reverse()
    unBindEvent()
    await transformed(dictList, cells)
    randomCallBack()
}

var left = async function () {
    let list = new NumberArray()
    let numbers = list.lineFlip
    let cells = extractedList(list.numbers)
    await move(numbers, cells)
}

var right = async function () {
    // 获得数字的二维数组
    ...
    let list = new NumberArray()
    let cells = extractedList(list.numbers)
    await move(list.numbers, cells)
}

var topMove = async function () {
    let list = new NumberArray()
...
var keyboardDown = function () {
    window.addEventListener('keydown', async event => {
        ...
        if (targetDict.loading) {
            log('上一步还没完结        ', targetDict.loading)
            return
        }
        const e = event
        const actionDict = {
        ...
        let action = actionDict[e.keyCode]
        action && await action()
    })
}
